(function (wp) {
	...
	registerBlockType('fv/modal-slider', {
		title: 'Big slider',
		icon: catIcon,
		category: 'common',
		keywords: ['Big slider', 'фке', 'art'],
		attributes: {
			title: {
				type: 'string',
				default: ''
	...
	function Edit(props) {
		...
		useEffect(() => {
			if (columns > blocksData.length) {
				...
				while (newBlocksData.length < columns) {
					newBlocksData.push({ title: '', description: '', image: '' });
				}
		...
		const blockItems = Array.from({ length: columns }, (x, index) => {
			const images = blocksData[index]?.images || [];
			const firstImageURL = images[0]?.url;
			return (
				<li key={index} className={'slide-modal slide item-' + (index + 1)} data-images={JSON.stringify(images.map(img => img.url))}>
					<MediaUpload
						allowedTypes={['image']}
						multiple={true}
						gallery={true}
						value={images.map(img => img.id)}
						onSelect={value => updateBlockData(index, 'images', value)}
						render={({ open }) => (
							<Button onClick={open} isPrimary isLarge>
								{ firstImageURL 
									? <img src={firstImageURL} alt={`First image`} width={"100%"} height={"auto"} /> 
									: 'Выбрать картинки' }
							</Button>
						)}
					/>
					{ isSelected && images.length > 0 && (
						<Button onClick={() => updateBlockData(index, 'images', [])} className={'remove-img'}>
							{`Удалить картинки`}
						</Button>
					) }
				</li>
