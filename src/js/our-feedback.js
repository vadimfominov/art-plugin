(function (wp) {
	...
	registerBlockType('fv/our-feedback', {
		title: 'Наши отзывы',
		icon: catIcon,
		category: 'common',
		keywords: ['Наши отзывы', 'our feedback', 'фке', 'art'],
		attributes: {
			title: {
				type: 'string',
				default: ''
			...
			blocksData: {
				type: 'array',
				default: [{ image: '', video: '', title: '', description: '', scrinshot: '' }],
		  	},
			columns: {
            type: 'number',
	...
	function Edit(props) {
		...
		const blockItems = Array.from({ length: columns }, (x, index) => {
			...
			const scrinshotURL = blocksData[index]?.scrinshot?.url;
	  
			return (
				 <li key={index} className={imageURL ? 'block-image slide item-' + (index + 1) : 'slide item-' + (index + 1)}>
					<div className={'slide-item'}>
						{/* Если видео загружено, скрываем текстовые поля и показываем кнопку "Добавить видео" */}
						{!!videoURL ? (
							<>
								...
								/>
								{isSelected && (
									<Button onClick={() => updateBlockData(index, 'video', null)} className={'remove-img'}>
										Удалить видео
									</Button>
								)}
							</>
							...
							<>
								{/* Если изображение загружено, скрываем текстовые поля и показываем кнопку "Добавить видео" */}
								{!!imageURL ? (
									<>
									...
									<>
										{/* Если ничего не загружено, показываем текстовые поля и кнопку для загрузки изображения */}
										...
										<MediaUpload
											allowedTypes={['image']}
											onSelect={value => updateBlockData(index, 'scrinshot', value)}
											render={({ open }) => (
												<Button className={'add-scrinshot'} onClick={open} isPrimary isLarge >
														{scrinshotURL ? 'Изменить скриншот' : 'Загрузить скриншот'}
												</Button>
											)}
										/>
										{scrinshotURL && (
											<Button onClick={() => updateBlockData(index, 'scrinshot', null)} className={'remove-img'}>
												Удалить скриншот
											</Button>
										)}
										{!scrinshotURL && !title && !description && <MediaUpload
											allowedTypes={['image']}
							...
							<ul className={classSlider + ' slider-block'}>
									{
										blocksData.map((block, index) => {
											...
											const parts = block.title && block.title.split('<br>');

											return (
												<li key={index} className={block.image?.url ? 'block-image slide item-' + (index + 1) : 'slide item-' + (index + 1)}>
													<div className={'slide-item'}>
														{block.video?.url ? (
															<>
																{block.image?.url && (
															...
															<>
																...
																{block.scrinshot?.url && <button className={'add-scrinshot'} data-imgurl={block.scrinshot.url} >Скриншот отзыва</button> }
															</>
														)}
													</div>
												</li>
