(function (wp) {
	...
	registerBlockType("fv/our-destinations", {
		...
		attributes: {
			...
			campData: {
				type: 'array',
				default: [{ title: '', description: '', link: 'Подробнее', link2: '', old: '' }],
			},
			proftestData: {
				type: 'array',
				default: [{
		...
		const campItems = Array.from({ length: campColumns }, (x, index) => (
			<div key={index} className={'item'}>
				...
				<div className="container-span">
					...
					<RichText
						tagName="span"
						onChange={value => updatedCampData(index, 'link2', value)}
						value={campData[index]?.link2}
						placeholder="Подробнее..."
					/>
				</div>
			</div>
							...
							{campData.map((block, index) => (
								<div key={index} className={'item'}>
									...
									<div className={'container-span'}>
										...
										{block.link2 && <RichText.Content tagName="span" value={block.link2} />}
									</div>
								</div>
							))}
						</div>
