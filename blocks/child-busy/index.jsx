(function (wp) {
	...
	registerBlockType("fv/child-busy", {
		apiVersion: 3,
		title: 'Ребёнок всегда занят',
		icon: catIcon,
		category: 'common',
						...
						render={({ open }) => (
							<Button onClick={open}>
								{blocksData[index]?.img?.url
									? <img src={blocksData[index]?.img.url} alt={'photo'} style={{ width: '100%', height: 'auto' }} />
									: 'Выбрать картинку'
								}
							</Button>
						)}
	...
	function Save(props) {
		const { title, description, blocksData, inBlock } = props.attributes;
		if (!inBlock) {
			return null;
		}
