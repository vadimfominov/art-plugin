(function (wp) {
	...
	const iconForSlide1 = (<svg width="25" height="17" viewBox="0 0 25 17" fill="none" xmlns="http://www.w3.org/2000/svg">
		<path d="M15.2344 0.78125H0.390625C0.174889 0.78125 0 0.956139 0 1.17187V16.0156C0 16.2314 0.174889 16.4062 0.390625 16.4062H15.2344C15.4501 16.4062 15.625 16.2314 15.625 16.0156V1.17187C15.625 0.956139 15.4501 0.78125 15.2344 0.78125Z" fill="#ffffff" fillOpacity="0.3" />
		<path d="M33.9844 0.78125H19.1406C18.9249 0.78125 18.75 0.956139 18.75 1.17187V16.0156C18.75 16.2314 18.9249 16.4062 19.1406 16.4062H33.9844C34.2001 16.4062 34.375 16.2314 34.375 16.0156V1.17187C34.375 0.956139 34.2001 0.78125 33.9844 0.78125Z" fill="#ffffff" fillOpacity="0.3" />
	</svg>);
	const iconForSlide2 = (<svg width="19" height="24" viewBox="0 0 19 24" fill="none" xmlns="http://www.w3.org/2000/svg">
		<path d="M6.16859 23.1016C6.16859 23.1016 2.95765 17.3828 1.60609 14.9063C-0.190788 11.6094 -0.487672 10.2188 0.754524 9.53125C1.52796 9.10156 2.59827 9.28125 3.25452 10.4219L4.78577 12.8594V3.30469C4.78577 3.30469 4.69202 0.78125 6.4889 0.78125C8.40297 0.78125 8.23891 3.30469 8.23891 3.30469V7.94531C8.23891 7.94531 9.24672 7.21875 10.4264 7.54688C11.028 7.71094 11.7311 8 12.1061 8.95313C12.1061 8.95313 14.5045 7.78906 15.6998 10.2656C15.6998 10.2656 18.4655 9.71875 18.4655 12.5859C18.4655 15.4531 15.0123 23.1016 15.0123 23.1016H6.16859Z" fill="#FFFFFF" />
	</svg>);
	...
	registerBlockType("fv/new-slider", {
		apiVersion: 3,
		title: 'Новый слайдер',
		icon: catIcon,
		category: 'common',
		keywords: ['new-slider', 'фке', 'art'],
		attributes: {
			title: {
				type: 'string',
				default: ''
	...
	function Edit(props) {
		...
		const { title, description, classSlider, gallery, imageDescriptions } = attributes;
		...
		useEffect(() => {
			...
			const timer = setTimeout(() => {
				if (typeof Swiper !== 'undefined' && containerRef.current && gallery.length > 0) {
		...
		return [
			...
			<div {...blockProps}>
				<div className="wrapper">
					...
					/>
					<div className={'slider-container my-swiper-container'} ref={containerRef} data-slider={classSlider}>
						<div className={'icons-for-slider'}>
							<span className={'icon-1'}>{iconForSlide1}</span>
							<span className={'icon-2'}>{iconForSlide2}</span>
						</div>
						<div className={classSlider + ' swiper'}>
							<ul className={'swiper-wrapper'}>
								{gallery.length > 0 && gallery.map((image, index) => (
									<li key={image.id || index} className={'item-' + (index + 1) + ' swiper-slide'}>
										<img
											src={image.url}
											alt={image.alt || ''}
	...
	function Save(props) {
		const { title, description, classSlider, gallery, imageDescriptions } = props.attributes;
		...
		return (
			<div {...blockProps}>
				<div className="wrapper">
					...
					<div className={'slider-container my-swiper-container'} data-slider={classSlider}>
						<div className={'icons-for-slider'}>
							<span className={'icon-1'}>{iconForSlide1}</span>
							<span className={'icon-2'}>{iconForSlide2}</span>
						</div>
							...
							<ul className={'swiper-wrapper'}>
								{gallery.length > 0 && gallery.map((image, index) => (
									<li key={image.id || index} className={'item-' + (index + 1) + ' swiper-slide'}>
										...
										/>
										{image.caption && (
											<RichText.Content tagName="span" value={image.caption} />
										)}
										{imageDescriptions[image.id] && (
											<RichText.Content tagName="p" value={imageDescriptions[image.id]} />
										)}
									</li>
								))}
							</ul>
