(function (wp) {
	...
	function Edit(props) {
		...
		const activeTab0 = (inActiveTab0 && !inActiveTab && !inActiveTab2) || (inActiveTab0 && inActiveTab) || (inActiveTab0 && inActiveTab2) || (inActiveTab0 && inActiveTab && inActiveTab2) ? 'active' : '';
		const activeTab = (inActiveTab && !inActiveTab0 && !inActiveTab2) || (inActiveTab && inActiveTab2) ? 'active' : '';
		const activeTab2 = (inActiveTab2 && !inActiveTab0 && !inActiveTab) ? 'active' : '';
		...
		return [
			<InspectorControls>
				...
				>
					...
					<PanelRow>
						<p>Включить вкладки 1</p>
						<ToggleControl
							checked={inActiveTab0}
							onChange={() => setAttributes({ inActiveTab0: !inActiveTab0 })}
			...
			<div {...blockProps}>
				<div className={'wrapper'}>
					...
					/>
					{((inActiveTab0 && inActiveTab) || (inActiveTab0 && inActiveTab2) || (inActiveTab && inActiveTab2)) && <div className={'select-block'}>
						<ul className={'select-ul'}>
							{inActiveTab0 && <li data-slug={'first'} className={'active'} onClick={handleTabClick}>
								<RichText
									tagName="span"
							...
							{inActiveTab && <li data-slug={'second'} onClick={handleTabClick}>
								<RichText
									tagName="span"
					...
					<div className={'content-tab'}>
						{inActiveTab0 && <div className={`item-tab first ${activeTab0}`}>
						...
						{inActiveTab && <div className={`item-tab second ${activeTab}`}>
						...
						{inActiveTab2 && <div className={`item-tab third ${activeTab2}`}>
		...
		return (
			<div {...blockProps}>
				<div className={'wrapper'}>
					...
					{((inActiveTab0 && inActiveTab) || (inActiveTab0 && inActiveTab2) || (inActiveTab && inActiveTab2)) && <div className={'select-block'}>
						<ul className={'select-ul'}>
							{inActiveTab0 && <li data-slug={'first'} className={'active'}>
								<RichText.Content tagName="span" value={title_tab1} />
							</li>}
							{inActiveTab && <li data-slug={'second'}>
